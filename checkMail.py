...
# ---> Helper functions
def addMessages(service,messageIds):
    i, l, batch = 0, len(messageIds), service.new_batch_http_request()
    while i < l:
        for messageId in messageIds[i:i+100]:
...
def _updateDb(request_id, response, exception):
...
def storeLastHistoryId(lastMessageId=None,lastHistoryId=None):
    if lastMessageId:
        q = s.query(HeaderInfo).get(lastMessageId)
...
def updateDb(service, lastHistoryId=None):
    ...
    if lastHistoryId is None:
        messageIds = ListMessagesMatchingQuery(mkService(),
                        'me',query='newer_than:3m')
        addMessages(service, [m['id'] for m in messageIds])
        ...
        addMessages(service, messagesAdded)
